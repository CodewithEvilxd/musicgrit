...
pub async fn status(playlist: Option<&str>, grit_dir: &Path) -> Result<()> {
    ...
    if !snapshot_path.exists() {
        bail!("Playlist not initialized. Run 'musicgrit init' first.");
    }
    ...
    if staged_patch.changes.is_empty() {
        ...
        println!("\nUse 'musicgrit commit -m \"message\"' to commit these changes");
        println!("Use 'musicgrit reset' to discard staged changes");
    }
    ...
    match provider.fetch(playlist_id).await {
        std::result::Result::Ok(remote_snapshot) => {
            ...
            if local_vs_remote.changes.is_empty() {
                ...
                println!("\n  Use 'musicgrit push' to sync with remote");
            }
        }
        Err(e) => {
...
pub async fn search(
    ...
    if add_mode && total_added > 0 {
        ...
        println!("Use 'musicgrit status' to see all staged changes");
        println!("Use 'musicgrit commit -m \"message\"' to commit");
    } else if !add_mode {
        println!("Use 'musicgrit add <track-id>' to stage a track for addition");
        println!(
            "Or use 'musicgrit search \"{}\" --add' for interactive mode",
            query
        );
    }
...
pub async fn add(track_id: &str, playlist: Option<&str>, grit_dir: &Path) -> Result<()> {
    ...
    let snapshot_path = snapshot::snapshot_path(grit_dir, playlist_id);
    if !snapshot_path.exists() {
        bail!("Playlist not initialized. Run 'musicmusicgrit init' first.");
    }
    ...
    println!("\nUse 'musicgrit status' to see all staged changes");
    println!("Use 'musicgrit commit -m \"message\"' to commit");
...
pub async fn commit(message: &str, playlist: Option<&str>, grit_dir: &Path) -> Result<()> {
    ...
    println!("Use 'musicgrit push' to sync with remote.");
